...
function ValidateDateForReservation(date) {
  ...
  return new Promise(async (done, reject) => {
    ...
    let dividedPerMinutes = await global.services.systemOptions.getOption('timeDividedPerMitutes');

    // get periods
    let periodCollection = global.services.contentProvider.getCollection('cms', 'period');
...
function getScheduleOptions() {
  return new Promise(async (done, reject) => {
    ...
    // get total days
    await global.services.systemOptions.getOption('totalAllowedDaysForReservation')
      .then(days => {
        totalDays = days
      })
      .catch((error) => {
    ...
    // get periods
    await global.services.contentProvider.getCollection('cms', 'period')
      .find({}).exec()
...
function getTables() {
  return global.services.contentProvider.getCollection('cms', 'table')
    .find({}).exec();
}
...
function getReservedTimes(ISODay, tableId) {
  ...
  return new Promise(async (done, reject) => {
    ...
    let typeCasters = global.services.contentProvider.TypeCasters;
    ...
    let ReservedTableCollection = global.services.contentProvider.getCollection('user', 'reservedTable');
    let tableModel = global.services.contentProvider.getCollection('cms', 'table');
...
function getRemainPersons(ISOdateTimeString, tableId) {
  ...
  let tableModel = global.services.contentProvider.getCollection('cms', 'table');
  let reservedTableModel = global.services.contentProvider.getCollection('user', 'reservedTable');
...
function cancel(reservedId) {
  // reserve table
  let reservedModel = global.services.contentProvider.getCollection('user', 'reservedTable');
...
function reserve(refId, ISOdateTimeString, tableId, persons) {
  ...
  return new Promise(async (done, reject) => {
    ...
    let additionMinutes = await global.services.systemOptions.getOption('timeDividedPerMitutes');
    ...
    // validate tableID
    let tableCollection = global.services.contentProvider.getCollection('cms', 'table');
